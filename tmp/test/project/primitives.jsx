...
const Icon = ({ name, size = 20, color = 'currentColor', strokeWidth = 1.6 }) => {
  ...
  const paths = {
    ...
    google: <><path d="M21.35 11.1h-9.17v2.96h5.24c-.22 1.18-.88 2.18-1.87 2.85v2.37h3.02c1.77-1.63 2.78-4.03 2.78-6.87 0-.56-.05-1.1-.14-1.61z" fill="#4285F4" stroke="none"/><path d="M12.18 21c2.52 0 4.64-.83 6.18-2.26l-3.02-2.37c-.84.56-1.91.9-3.16.9-2.43 0-4.49-1.64-5.22-3.85H3.84v2.42C5.38 18.9 8.55 21 12.18 21z" fill="#34A853" stroke="none"/><path d="M6.96 13.42a5.4 5.4 0 0 1 0-3.44V7.56H3.84a8.99 8.99 0 0 0 0 8.28z" fill="#FBBC05" stroke="none"/><path d="M12.18 6.13c1.37 0 2.6.47 3.57 1.4l2.67-2.67C16.81 3.47 14.7 2.6 12.18 2.6c-3.63 0-6.8 2.1-8.34 5.16l3.12 2.42c.73-2.21 2.79-3.85 5.22-3.85z" fill="#EA4335" stroke="none"/></>,
...
const Wordmark = ({ size = 'md', color }) => {
  ...
  const fs = sizes[size];
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: color || S.primary, fontWeight: 600, letterSpacing: '-0.02em', fontSize: fs }}>
      <div style={{
        width: fs + 6, height: fs + 6, borderRadius: 8,
...
// ========== BUTTONS ==========
const Button = ({ children, variant = 'primary', size = 'md', icon, iconRight, fullWidth, disabled, destructive, style, ...rest }) => {
  const sizes = {
  ...
  const common = {
    ...
    borderRadius: 9999, fontWeight: 500, cursor: disabled ? 'default' : 'pointer',
    border: 'none', letterSpacing: '-0.005em', whiteSpace: 'nowrap',
...
const TypeBadge = ({ type }) => {
  ...
  return <Badge color={map[type] || 'neutral'} dot>{type}</Badge>;
};
...
const StatusBadge = ({ status }) => {
  ...
  return <Badge color={s.c} dot>{s.label}</Badge>;
};
...
const Avatar = ({ name = '', size = 36, src, color }) => {
  ...
  return (
    <div style={{
      ...
      color: bg, display: 'grid', placeItems: 'center',
    ...
    <div style={{ flex: 1, minWidth: 0 }}>
      {overline && (
        <div style={{
          ...
          textTransform: 'uppercase', color: S.onSurfaceMuted, marginBottom: 10,
        }}>{overline}</div>
      )}
      <h1 style={{
        fontSize: 36, fontWeight: 600, letterSpacing: '-0.025em',
        color: S.onSurface, margin: 0, lineHeight: 1.1,
      }}>{title}</h1>
      {subtitle && (
        <div style={{ fontSize: 15, color: S.onSurfaceVariant, marginTop: 10, maxWidth: 640, lineHeight: 1.5 }}>
          {subtitle}
        </div>
      )}
    </div>
...
// ========== STAT CARD ==========
const StatCard = ({ label, value, caption, delta, deltaDirection, icon, accent }) => (
  <Card padding={24}>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
      <div style={{
        fontSize: 11, fontWeight: 600, letterSpacing: '0.08em',
        textTransform: 'uppercase', color: S.onSurfaceMuted,
      }}>{label}</div>
      {delta && (
    ...
    <div style={{
      ...
      color: S.onSurface, fontVariantNumeric: 'tabular-nums', lineHeight: 1,
    ...
    {caption && <div style={{ fontSize: 13, color: S.onSurfaceMuted, marginTop: 10 }}>{caption}</div>}
  </Card>
);
...
const Amount = ({ value, size = 'row', currency = '$', gradient }) => {
  ...
  return (
    <span style={{
      fontSize: sizes[size], fontWeight: size === 'display' ? 600 : 500,
      ...
      <span style={{ opacity: 0.6, marginRight: 2 }}>{currency}</span>{value}
    </span>
  );
};

// ========== INPUT ==========
const Input = ({ label, icon, value, placeholder, helper, error, prefix, suffix, fullWidth = true, style }) => (
  <div style={{ width: fullWidth ? '100%' : undefined }}>
    ...
    <div style={{
      ...
      color: S.onSurface, ...style,
    }}>
      {icon && <Icon name={icon} size={16} color={S.onSurfaceMuted} />}
      {prefix && <span style={{ color: S.onSurfaceMuted, fontSize: 14 }}>{prefix}</span>}
      <span style={{ flex: 1, fontSize: 14, color: value ? S.onSurface : S.onSurfaceMuted, fontVariantNumeric: 'tabular-nums' }}>
        {value || placeholder}
      </span>
      {suffix && <span style={{ color: S.onSurfaceMuted, fontSize: 13 }}>{suffix}</span>}
    </div>
    {helper && !error && <div style={{ fontSize: 12, color: S.onSurfaceMuted, marginTop: 6 }}>{helper}</div>}
...
// ========== SIDEBAR ==========
const Sidebar = ({ role = 'admin', active, churchName = 'Grace Community', userName = 'Sarah Chen' }) => {
  const navs = {
    admin: [
  ...
  const roleLabel = role === 'admin' ? 'Admin' : role === 'super' ? 'Super' : 'Member';
  return (
    <div style={{
      width: 260, height: '100%', background: S.surfaceContainerLowest,
      ...
      <div style={{
        ...
        borderRadius: 12, background: S.surfaceContainerLow, marginBottom: 20,
      }}>
        <div style={{
          ...
          {churchName.split(' ').map(w => w[0]).join('').slice(0, 2)}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: S.onSurface, letterSpacing: '-0.01em',
            overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{churchName}</div>
          <div style={{ fontSize: 11, color: S.onSurfaceMuted }}>Switch church</div>
        </div>
      ...
      <div style={{
        display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px',
        borderRadius: 12, background: S.surfaceContainerLow, marginTop: 16,
      }}>
        <Avatar name={userName} size={32} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: S.onSurface,
...
const TopBar = ({ breadcrumb = 'Dashboard', showSwitcher }) => (
  <div style={{
    ...
    <div style={{
      ...
      background: S.surfaceContainerLowest, borderRadius: 9999,
      ...
      <span style={{ fontSize: 11, color: S.onSurfaceMuted, marginLeft: 'auto',
        background: S.surfaceContainer, padding: '2px 8px', borderRadius: 6 }}>⌘K</span>
    </div>
    <div style={{
      width: 40, height: 40, borderRadius: '50%', background: S.surfaceContainerLowest,
      display: 'grid', placeItems: 'center', position: 'relative',
    }}>
      <Icon name="bell" size={18} color={S.onSurfaceVariant} />
      <span style={{
        position: 'absolute', top: 8, right: 10, width: 8, height: 8,
        borderRadius: '50%', background: S.tertiary, border: `2px solid ${S.surfaceContainerLowest}`,
      }} />
    </div>
  </div>
);
...
const AppShell = ({ role, active, breadcrumb, churchName, userName, children, contentPad = 32, bg }) => (
  <div style={{
    ...
    <Sidebar role={role} active={active} churchName={churchName} userName={userName} />
    <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <TopBar breadcrumb={breadcrumb} />
      <div style={{ flex: 1, padding: `0 ${contentPad}px ${contentPad}px`, overflow: 'hidden' }}>
        {children}
      </div>
    </div>
  </div>
...
const Table = ({ columns, rows }) => (
  <div style={{ background: S.surfaceContainerLowest, borderRadius: 16, padding: '8px 0', overflow: 'hidden' }}>
    {/* Header */}
    <div style={{
    ...
    {rows.map((row, i) => (
      <div key={i} style={{
        ...
        borderRadius: 12, margin: '2px 8px',
        background: row._hover ? S.surfaceContainerLow : 'transparent',
        fontSize: 14, color: S.onSurface, minHeight: 40,
      }}>
        {columns.map((c, j) => (
...
const SectionTitle = ({ title, action }) => (
  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
    <h3 style={{ fontSize: 18, fontWeight: 600, letterSpacing: '-0.02em', color: S.onSurface, margin: 0 }}>{title}</h3>
    {action}
  </div>
);

Object.assign(window, {
  Icon, Wordmark, Button, Badge, TypeBadge, StatusBadge, Avatar,
  PageHeader, Card, StatCard, Amount, Input, Chip,
  Sidebar, TopBar, AppShell, Table, SectionTitle,
});
