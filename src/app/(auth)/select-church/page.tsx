...
export default async () => {
  ...
  if (memberships.length === 1 && !user.isSuperAdmin) {
    ...
    redirect(role === "ADMIN" ? `/${slug}/admin/dashboard` : `/${slug}/member/dashboard`);
  }
  ...
  return (
    <AuthMarketingShell panel="plain-wide">
      ...
      {memberships.length > 0 && (
        <div
          className={
            memberships.length > 1 ? "mt-8 grid grid-cols-2 gap-4" : "mt-8 grid grid-cols-1 gap-4"
          }
        >
          {memberships.map(({ slug, role }) => (
            <Link
              key={slug}
              href={role === "ADMIN" ? `/${slug}/admin/dashboard` : `/${slug}/member/dashboard`}
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="text-base font-semibold tracking-tight">{slug}</div>
      ...
      {user.isSuperAdmin && (
        <div className="mt-8 rounded-xl bg-muted p-5 text-center">
          <div className="mb-2 text-[13px] text-muted-foreground">You&apos;re a platform admin.</div>
          <Link href="/super-admin/tenants" className="font-semibold text-primary no-underline hover:underline">
            Go to platform ops →
          </Link>
        </div>
      )}
