    ...
    <div className={cn("rounded-2xl bg-card p-2 overflow-visible", className)}>
              ...
              return (
                <TableRow
                  ...
                  className={cn(
                    ...
                    clickable ? "cursor-pointer hover:bg-muted/50" : "hover:bg-muted/20"
                  )}
                >
                  {columns.map((c) => (
                    <TableCell
                      ...
                      className={cn(
                        ...
                        c.align === "right" && "text-right",
                        c.align === "center" && "text-center",
                        c.overflow === "visible" ? "whitespace-normal" : "whitespace-nowrap truncate max-w-[200px]"
                      )}
                    >
                      {c.render(row)}
                    </TableCell>
      ...
      {!loading && (rows?.length ?? 0) === 0 && (
        <div className="py-20 px-6 text-center">
          ...
          {emptySubtitle && <p className="text-sm text-muted-foreground mb-6">{emptySubtitle}</p>}
          {emptyAction}
        </div>
      )}
  ...
  return (
    <div className="flex items-center justify-between px-6 py-4 mt-2 border-t border-border/40">
      <span className="text-[13px] font-medium text-muted-foreground">
        Showing <span className="text-foreground">{offset + 1}</span>–<span className="text-foreground">{last}</span> of <span className="text-foreground">{total}</span>
      </span>
