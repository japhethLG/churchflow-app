...
export const InviteMemberModal = ({
		...
		return (
			...
			>
				<p className="m-0 text-sm leading-relaxed text-secondary-foreground">
					{claiming ? (
						<>
							An invitation has been sent to <strong>{sentTo.email}</strong>.
							When they accept, their existing profile
							{claimMemberName ? ` for ${claimMemberName}` : ""} — including all
							giving history — will be linked to their account.
						</>
					) : (
						<>
							An invitation has been sent to <strong>{sentTo.email}</strong>.
							They&apos;ll join the church as{" "}
							{sentTo.role === "ADMIN" ? "an admin" : "a member"} once they sign
							in with the link.
						</>
					)}
				</p>
	...
	return (
		<BaseModal
			...
			title={
				claiming
					? `Invite ${claimMemberName ?? "member"} to claim profile`
					: "Invite member"
			}
			size="md"
			onClose={onClose}
		...
		>
			<Form methods={methods} onSubmit={onSubmit}>
				<p className="m-0 text-sm leading-relaxed text-secondary-foreground">
					{claiming ? (
						<>
							The invited person receives a Google sign-in link. When they
							accept, this temp profile is linked to their account — every
							transaction and pledge already recorded for this profile stays
							attached.
						</>
					) : (
						<>
							The invited person receives a Google sign-in link. After they
							accept, a member profile is created automatically and they can
							pledge or give.
						</>
					)}
				</p>
