...
export const ConfirmDeletePledgeModal = ({
	...
	const handleDelete = async () => {
		...
		try {
			await mutateAsync({
				params: { path: { tenantId: tenantSlug, id: pledgeId } },
			});
			onClose();
		} catch (err) {
	...
	return (
		...
		>
			<p className="m-0 text-sm leading-relaxed text-secondary-foreground">
				Removes this pledge from the campaign&apos;s totals. Use{" "}
				<strong>Edit → Cancelled</strong> instead if you want to keep the record
				visible.
			</p>
