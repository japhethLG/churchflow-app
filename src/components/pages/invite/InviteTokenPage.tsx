...
export const InviteTokenPage = ({ params }: { params: Params }) => {
    ...
    return (
      <AuthMarketingShell>
        ...
        <p className="text-[15px] leading-relaxed text-secondary-foreground">
          This invitation link has expired, been cancelled, or was already accepted. Ask an admin to send you a new
          invitation.
        </p>
        <div className="mt-7">
  ...
  if (inv.status === "ACCEPTED") {
    ...
    return (
      <AuthMarketingShell>
        <div className="mb-3 text-[11px] font-semibold uppercase tracking-wider text-tertiary">Already accepted</div>
        <h1 className="mb-3 text-[26px] font-semibold tracking-tight text-foreground">
          You&apos;ve already joined{inv.tenantName ? ` ${inv.tenantName}` : ""}.
        </h1>
        <p className="text-[15px] leading-relaxed text-secondary-foreground">This invitation has already been accepted.</p>
        {slug && (
          <div className="mt-7">
            <Button
              variant="primary"
              ...
              fullWidth
              onClick={() =>
                router.push(inv.role === "ADMIN" ? `/${slug}/admin/dashboard` : `/${slug}/member/dashboard`)
              }
            >
              Go to church
            </Button>
  ...
  return (
    <AuthMarketingShell>
      <div className="mb-3 text-[11px] font-semibold uppercase tracking-wider text-tertiary">Invitation</div>
      ...
      <p className="mt-3.5 text-[15px] leading-relaxed text-secondary-foreground">
        ...
        <strong className="text-foreground">{inv.role === "ADMIN" ? "Admin" : "Member"}</strong>. Sign in with Google to
        accept.
      </p>
      ...
      <div className="mt-7 flex gap-4 rounded-xl bg-muted p-4">
        <div className="flex min-w-0 flex-1 items-center gap-2.5">
          ...
          <div className="min-w-0">
            <div className="text-[13px] font-semibold">{inv.tenantName ?? "—"}</div>
        ...
        {inv.inviterDisplayName && (
          <>
            ...
            <div className="flex min-w-0 flex-1 items-center gap-2.5">
              ...
              <div className="min-w-0">
                <div className="text-[13px] font-semibold">{inv.inviterDisplayName}</div>
      ...
      {firebaseUser && (
        <div className="mt-4 flex items-center gap-2.5 rounded-[10px] bg-muted px-3.5 py-2.5 text-[13px]">
          ...
          <span className="min-w-0 flex-1 text-secondary-foreground">
            Accepting as <strong className="text-foreground">{firebaseUser.displayName ?? firebaseUser.email}</strong>
          </span>
          <Button
            type="button"
            variant="tertiary"
      ...
      {actionError && <p className="mt-3 text-[13px] text-destructive">{actionError}</p>}
