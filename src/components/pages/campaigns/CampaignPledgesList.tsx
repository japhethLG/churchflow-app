...
export const CampaignPledgesList = ({
  ...
  const itemById: Record<string, Item> = Object.fromEntries(items.map((i) => [i.id, i]));
  const memberById: Record<string, { firstName: string; lastName: string }> = Object.fromEntries(
    (membersData?.items ?? []).map((m) => [m.id, { firstName: m.firstName, lastName: m.lastName }])
  );
      ...
      render: (p) => (
        <RowActionsMenu
          actions={[
            ...
            { label: "Delete", onClick: () => onDelete(p), destructive: true, separatorBefore: true },
          ]}
        />
      ),
    },
