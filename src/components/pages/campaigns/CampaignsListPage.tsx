...
export const CampaignsListPage = () => {
  ...
  const [filters, setFilters] = useState<CampaignsFiltersValue>({ search: "", status: "all" });
  ...
  const goView = (c: CampaignRow) => router.push(`/${tenantSlug}/admin/campaigns/${c.id}`);
  const goEdit = (c: CampaignRow) => router.push(`/${tenantSlug}/admin/campaigns/${c.id}/edit`);
  const askCancel = (c: CampaignRow) =>
    openModal("confirm-cancel-campaign", { tenantSlug, campaignId: c.id, campaignTitle: c.title });
  const askDelete = (c: CampaignRow) =>
    openModal("confirm-delete-campaign", { tenantSlug, campaignId: c.id, campaignTitle: c.title });
  ...
  return (
    <div className="h-full overflow-auto">
      ...
      <CampaignsTable
        rows={visible}
        loading={isLoading}
        pagination={{ total: filtered.length, offset, limit: PAGE_SIZE, onChange: setOffset }}
        handlers={{
          onView: goView,
          onEdit: goEdit,
          onCancel: askCancel,
